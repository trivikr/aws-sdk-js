...
AWS.TokenFileWebIdentityCredentials = AWS.util.inherit(AWS.Credentials, {
  ...
  /**
   * Returns web_identity_token_file from source profile
   *
   * @api private
   */
  getTokenFileFromSourceProfile: function getTokenFileFromSourceProfile(profiles, profileName) {
    var sourceProfileName = profiles[profileName]['source_profile'];
    var sourceProfile = profiles[sourceProfileName];
    if (sourceProfile) {
      return sourceProfile['web_identity_token_file'];
    } else {
  ...
  getParamsFromSharedConfig: function getParamsFromSharedConfig() {
    ...
    // if web_identity_token_file is absent, read from source_profile
    if (!profile['web_identity_token_file']) {
      profile['web_identity_token_file'] = this.getTokenFileFromSourceProfile(profiles, profileName);
    }
