...
AWS.TokenFileWebIdentityCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   * @api private
   */
  getParamsFromSharedConfig: function getParamsFromSharedConfig(name) {
    var profiles = AWS.util.getProfilesFromSharedConfig(iniLoader);
    var profileName = name || process.env.AWS_PROFILE || AWS.util.defaultProfile;
  ...
  load: function load(callback) {
    try {
      ...
      if (params) {
        this.getCredentialsFromOidcToken(params, callback);
      } else {
        params = this.getParamsFromSharedConfig();
        if (params.envTokenFile) {
          this.getCredentialsFromOidcToken(params, callback);
        } else if (params.sourceProfile) {
          // Chaining
          var sourceProfileParams;
          var currentProfileParams = params;
          while (!sourceProfileParams.envTokenFile) {
            // Chaining happens here - difficult to do with loop/recursion
          }
        }
      }
      // Populate credentials from this.data
      this.service.credentialsFrom(this.data, this);
    } catch (err) {
      callback(err);
    }
  },
   ...
   * @api private
   */
  getCredentialsFromOidcToken: function getCredentialsFromOidcToken(params, callback) {
    ...
    this.callAssumeRoleWithWebIdentity(callback);
  },

  /**
   * @api private
   */
  callAssumeRoleWithWebIdentity: function callAssumeRoleWithWebIdentity(callback) {
    var self = this;
   ...
   * @api private
   */
  callAssumeRole: function callAssumeRole(callback) {
    var self = this;
    self.service.assumeRole(function (err, data) {
      self.data = null;
      if (!err) {
        self.data = data;
      }
